...
const MMpage = () => {
  ...
  const handleDownloadAll = async (contributionId) => {
    try {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const filteredData = response.data.filter(item => item.approval === true);
    ...
    <div>
      {isAuthorized(2) ? (
        <>
          ...
          <Button style={{marginTop: '15px'}} onClick={() => {handleDownloadAll()}}>Down ALL</Button>
        </>
      ) : (
        <div>
